...
<body style="text-transform:uppercase; font-size: 22px; font-family:  arial,sans-serif">
	<div id="middel">
	...
	<?php
		...
		<form action="upload.php" method="POST" name="loadnuwe" id="formtoets" onsubmit="return required()" enctype="multipart/form-data">
			<div style=" text-align: center;"><p style="line-height: 40px,  margin-top:5px; margin-bottom: 5px;">Test Name: </p><input type="text" name='toetsnaam' id="toetsnaam" style="height:30; width:300px"/></div><br>
		...
		<?php
			...
			for($x=0;$x<=100;$x++)
			{
				...
				if($x==0)
					{
?>
			<div id="altydhidden<?php echo $x;?>" style="display: block;" class="fktg1">
	...
	<div onClick="wysmuli<?php echo $x;?>()" id="wysmuli1<?php echo $x;?>" class="vraagtipe2">Multiple Choose</div>		
	<div onClick="wysintik<?php echo $x;?>()" id="vraaghideintik<?php echo $x;?>" class="vraagtipe1"> Type in Question</div>
				</div>
	<div id="hden<?php echo $x;?>" style="display: block;" class="bordertoets1">
<?php	
					}
				else
					{
?>
	<div id="altydhidden<?php echo $x;?>" style="display: none;" class="fktg2">
	<label class="containerfoto"><input type="file" name="fileToUpload<?php echo $x;?>" id="fileToUpload<?php echo $x;?>">
		<img class="fotospan" id="fotospan<?php echo $x;?>" style="height: 150px; width: 150px; background: url('addimage.png'); background-size: 150px 150px; background-repeat: no-repeat"/>
	</label>
		<div onClick="wysmuli<?php echo $x;?>()" id="wysmuli1<?php echo $x;?>" class="vraagtipe2">Multiple Choose</div>		
	<div onClick="wysintik<?php echo $x;?>()" id="vraaghideintik<?php echo $x;?>" class="vraagtipe1"> Type in Question</div>		
		...
		<div id="hden<?php echo $x;?>" style="display: none"  class="bordertoets2">
						<?php
					}
					?>
...
<script>
	...
	file<?php echo $x;?>.addEventListener('change',function() {
  	...
  	if(image<?php echo $x;?>) reader<?php echo $x;?>.readAsDataURL(image<?php echo $x;?>);
			...
			<p style="margin: 10px 0px 10px 0px">Question <?php echo $x+1;?>:</p><textarea name="vraag<?php echo $x;?>" type="text" id="vraag<?php echo $x;?>" class="rq" cols="60" rows="7"></textarea>
		<?php
				for($r=0;$r<=4;$r++)
				{
		?>
					<p style="margin: 10px 0px 10px 0px">Option <?php echo $r+1;?>:</p><textarea id="opsie<?php echo $x , $r;?>" name="opsie<?php echo $x , $r;?>" type="text" onkeydown="inputfunction<?php echo $x , $r;?>()" onkeyup="inputfunction<?php echo $x , $r;?>()" value="" class="rq" cols="35" rows="5"></textarea></br>
		<?php
				} 
		?>
	<p style="margin: 10px 0px 10px 0px">Answer :</p><select id="sel<?php echo $x ;?>" name="sel<?php echo $x;?>" class="soflow" onChange="wysAnder<?php echo $x;?>()">
				<?php
					for($v=0;$v<=4;$v++)
						{
				?>
...
<script>
	function wysintik<?php echo $x; ?>()
	{
		...
		<?php 
				...
				{
					?>
					document.getElementById("wysmuli1<?php echo $p;?>").style.opacity = "0.5";
					document.getElementById("vraaghideintik<?php echo $p;?>").style.opacity = "1";
					<?php
				}
				?>
			}
					...
					function inputfunction<?php echo $x , $q;?>() 
						{
							...
							document.getElementById("altydhidden<?php echo $x+1;?>").style.display = "block";
...
<script>	
	...
	function wysmuli<?php echo $x; ?>()
	{
		...
		document.getElementById("hden<?php echo $x;?>").style.display = 'block';
		document.getElementById("vraaghideintik<?php echo $x;?>").style.opacity = "0.5";
		document.getElementById("wysmuli1<?php echo $x;?>").style.opacity = "1";
	}
</script>
<div id="hdenintik<?php echo $x;?>" style="display: none"  class="bordertoets">
				

	<p style="margin: 10px 0px 10px 0px">Question <?php echo $x+1;?>:</p>
	<textarea name="vraagl<?php echo $x;?>" type="text" id="vraagl<?php echo $x;?>" class="rq" cols="60" rows="7"></textarea>

				<p style="margin: 10px 0px 10px 0px">Answer :</p>
<textarea id="antwintik<?php echo $x;?>" name="antwintik<?php echo $x ;?>" type="text" onkeyup="inputfunctionintik<?php echo $x;?>()" class="rq" cols="35" rows="5"></textarea>
	...
	?><input name="submit" type="submit" value="Submit" id="submitbutton"/>
