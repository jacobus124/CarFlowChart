...
#new
{
  color:#b81d15
    ...
    <script>
        function Addsql(AppName,AppLocation)
        {
          if(AppName.length>0 && AppLocation.length>0)
          {
            var xmlhttp = new XMLHttpRequest();
		        xmlhttp.open("GET", "sqlinleesLocationChange.php?q=<?php echo $com?>||" + AppName + "||" + AppLocation  , true);
      ...
      function NewApp()
      {
        ...
        document.getElementById("allappdiv").style.display="none";
      }
      function App()
      {
        document.getElementById("detaillist").style.display="none";
        document.getElementById("allappdiv").style.display="block";
      }
    </script>
  </head>
  <body>
      <ul id="mainentry">
        <li><span class="Opskrifte">User</span></br>
            <select id="AppName" name="AppName" style="width:200px; margin: 0 0 10px 0;">
                <?php
        ...
        <li><input type="Submit" value="Submit" onClick="Addsql(AppName.value,AppLocation.value)" class="SubmitAllClass"/></li>
      </ul>
    <input
      id="pac-input"
      class="dertig"
